...
import CoordDisplay from '@/components/analysis/CoordDisplay';
import { Square, Map as MapIcon, Satellite, Mountain, Moon, Box, X, MousePointer2 } from 'lucide-react';
...
function makeRasterStyle(id: string, tiles: string[], attribution: string) {
  return {
    ...
    layers: [{ id, type: 'raster' as const, source: id, minzoom: 0, maxzoom: 19 }],
  };
}
...
export default function WorldMap() {
  ...
  /* ---- Camera requests from search / shortcuts ---- */
  useEffect(() => {
    if (!flyTo) return;
    mapRef.current?.flyTo({ center: flyTo.center, zoom: flyTo.zoom, duration: 1400 });
  }, [flyTo]);
  ...
  const handleMouseMove = useCallback(
    (e: MapLayerMouseEvent) => {
      ...
      if (drawStart) setDrawCurrent({ lng: e.lngLat.lng, lat: e.lngLat.lat });
    },
  ...
  const handleMouseUp = useCallback(
    (e: MapLayerMouseEvent) => finishDraw(e.lngLat.lng, e.lngLat.lat),
    [finishDraw],
  );

  const handleMouseLeave = useCallback(() => setCursorCoord(null), [setCursorCoord]);

  /* ---- Escape cancels an in-progress draw ---- */
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key !== 'Escape' || !drawStart) return;
      setDrawStart(null);
      setDrawCurrent(null);
    };
  ...
  /* ---- Global mouseup for when the cursor leaves the map while drawing ---- */
    ...
    <div className="relative h-full w-full">
      ...
      >
        ...
        <Source id="drawing-src" type="geojson" data={drawingFC}>
          <Layer id="drawing-fill" type="fill" paint={{ 'fill-color': '#3b82f6', 'fill-opacity': 0.1 }} />
          <Layer
            id="drawing-line"
            type="line"
        ...
        <Source id="selection-src" type="geojson" data={selectionFC}>
          <Layer id="selection-fill" type="fill" paint={{ 'fill-color': '#22c55e', 'fill-opacity': 0.12 }} />
          <Layer id="selection-line" type="line" paint={{ 'line-color': '#16a34a', 'line-width': 2 }} />
        </Source>
      </Map>

      {/* Selection tools — top left */}
      <div className="absolute left-3 top-3 flex flex-col gap-1 rounded-lg bg-zinc-900/90 p-1 shadow-lg backdrop-blur-md">
        <ToolBtn Icon={MousePointer2} label="Pan" active={selectionMode === null} onClick={() => setSelectionMode(null)} />
        <ToolBtn
          Icon={Square}
          label="Select region (draw a box)"
      ...
      <div className="absolute right-3 top-28 flex flex-col gap-1 rounded-lg bg-zinc-900/90 p-1 shadow-lg backdrop-blur-md">
        {BASEMAP_META.map(({ key, label, Icon }) => (
          <ToolBtn key={key} Icon={Icon} label={label} active={basemap === key} onClick={() => setBasemap(key)} />
        ))}
      </div>

      {/* Coordinates — bottom left, above the scale bar */}
      <CoordDisplay className="absolute bottom-10 left-3 shadow-lg" />

      {/* Build twin / clear — bottom right */}
      {selectedRegion && (
        <div className="absolute bottom-6 right-3 flex items-center gap-2">
          <button
            ...
            className="flex cursor-pointer items-center gap-1.5 rounded-lg bg-zinc-900/90 px-3 py-2.5 text-sm font-medium text-zinc-300 shadow-lg backdrop-blur-md transition-colors hover:bg-zinc-800"
          ...
          <button
            ...
            className="flex cursor-pointer items-center gap-2 rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg transition-colors hover:bg-blue-500"
          >
            <Box size={16} />
            Build digital twin
          </button>
      ...
      {!selectedRegion && selectionMode !== 'bbox' && (
        <div className="pointer-events-none absolute left-1/2 top-3 -translate-x-1/2 select-none rounded-full bg-zinc-900/85 px-4 py-1.5 text-xs font-medium text-zinc-300 shadow-lg backdrop-blur-md">
          Search a place above, or draw a box to build its digital twin
        </div>
      )}
    </div>
  );
